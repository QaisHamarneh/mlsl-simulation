"""File I/O utilities for RL results and models.

This module provides functions for saving and loading:
- Trained models
- Hyperparameters
...
│   ├── {scenario}/
│   │   ├── {algorithm}/            # e.g., PPO
│   │   │   ├── {observation}/      # e.g., NUMERIC_OBSERVATION
│   │   │   │   ├── {reward_type}/  # e.g., INITIAL_REWARD
│   │   │   │   │   ├── 2025-10-29 18:26:52/
│   │   │   │   │   │   ├── best_model
│   │   │   │   │   │   └── history/
│   │   │   │   │   │       └── HH:MM:SS_steps.pkl
...
path_center = get_path_center(scenario, algo, obs, reward)
model_path = get_complete_path(path_center, timestamp, model=True)
save_trained_model(trained_agent, model_path)

param_path = get_complete_path(path_center, timestamp, model=False)
save_best_params(best_hyperparams, param_path)

# Load for later use
loaded_model = load_best_model(path_center, timestamp, rl_algorithm, env)
loaded_params = load_best_params(path_center, timestamp)
```
"""
import os
import datetime
...
def load_best_model(path_center: str, id: str, rl_algorithm: RLAlgorithm, env: MlslEnv) -> BaseAlgorithm:
    ...
    Example:
        >>> path_center = get_path_center("circuit", "PPO", "NUMERIC_OBSERVATION", "INITIAL_REWARD")
        >>> model = load_best_model(path_center, "2025-10-29 18:26:52", ppo_algo, env)
    """
...
def load_best_params(path_center: str, id: str) -> Dict[str, Any]:
    ...
    Example:
        >>> params = load_best_params(path_center, "2025-10-29 18:26:52")
        >>> ppo_algo.change_params(params)
    """
