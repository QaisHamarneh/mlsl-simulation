...
class SafetyController:
    ...
    def safe_lanes(self) -> Tuple[int, int, int]:
        """
        ...
        if self.car.changing_lane:
            ...
            if right_lane_acceleration < MAX_ACC:
                ...
                if left_lane is not None:
                    ...
                    if not self._check_collision(left_segment) and self.get_accelerate([left_segment]) > MAX_DEC:
                        left_lane = True
                        
        return right_lane, current_lane, left_lane

    def max_accelerate(self, segments: list[SegmentInfo]) -> int:
        """
